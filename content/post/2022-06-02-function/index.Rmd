...
As we see, its one of the models. We can get more info into it by running summary function.

```{r}
summary(model_nested$model[[1]])
```

But why do summary separately, if we know we Purrr :)
...
We use ‘glance’ function to return all the output on each row-wise basis for each dataframe. We can now use this output straight to plot some nice figures
