...
model_nested_summarised<-model_nested |>  
  mutate(model_summary = map(.x = model, .f = ~summary(.) |>tidy()))
...
```
We can also get the summary for the first  group using the `pluck` function from purrr package.

```{r,include=TRUE}
model_nested_summarised |> pluck('model_summary',1)
```
